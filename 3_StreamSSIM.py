...
locked_ssim_trigger = 0.999             # running SSIM average above this amount will log a "locked" event, below will clear it
...
def detect_locked_stream(last_x_list, lock_detected):
    ...
    if (average_ssim > 0.999 and lock_detected == False):
        print("Lock detected: {}".format(datetime.now()))
        lock_detected = True
        locked_events += 1
    elif (average_ssim < 0.999 and lock_detected == True):
